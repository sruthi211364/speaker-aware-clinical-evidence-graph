...
class GroundingCitation(Base):
    """Links a claim (or, from Phase 5 on, a policy verdict) to a retrieved
    external reference -- a guideline snippet, a drug interaction record, or
    a prior encounter note. This is what turns a policy decision from "the
    model said so" into "the model said so, and here is the retrieved
    evidence it used." Populated starting in Phase 4."""
    ...
    # FK to policy_verdicts.id added once that table exists (Phase 5).
    verdict_id: Mapped[GUID | None] = mapped_column(GUID(), nullable=True)

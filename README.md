...
This runs a small golden dataset through claim extraction and the policy engine's status logic, and reports precision/recall/F1 plus status-accuracy. `--mode mock` scores the pipeline against hand-authored stand-in responses, proving the harness's own scoring logic and the app's status-derivation function are correct without spending a cent. `--mode live` calls Claude for real — this has now actually been run (see [limitations](#what-isnt-here-yet) for what it found, including two real bugs in the harness itself that mock mode structurally couldn't have caught).
...
4. Want to see live extraction instead of the seeded stand-ins? Add a funded `ANTHROPIC_API_KEY` to `.env`, restart the `api` container, create a new encounter, add a transcript, and hit **Run full pipeline** from **Audit & Lineage**. It runs the real thing end to end and pauses at the clinician-review interrupt for you to act on. This has actually been done: a two-line "headache" transcript correctly extracted into two claims, coded to the right SNOMED concept, linked by a sensible `refines` edge, and grounded into two well-targeted clarification questions citing the exact missing documentation elements -- see [limitations](#what-isnt-here-yet) for what that same live run exposed about the eval harness itself. Raw audio upload additionally needs a funded `ASSEMBLYAI_API_KEY`.
...
| `--mode live` doesn't fairly test the missing-context/clinical-safety checks | It always hands the policy check empty placeholder evidence rather than wiring up real RAG retrieval, so those two checks -- which are correctly grounding-dependent -- have nothing to ground against through the harness. The real pipeline (verified separately, see below) does retrieve real evidence and gets this right |
| Live results vary between runs | Claude's calls here don't pin a temperature, so the same golden example can extract or classify slightly differently across separate live calls -- expected LLM behavior, not a bug, but it means a single `--mode live` run is a snapshot, not a stable benchmark. The golden dataset (4 examples) is also small enough that no version of this would mean much statistically yet |
